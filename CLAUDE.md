...
- **Catalog bounded context:** [catalog/](backend/app/catalog/) — domain dataclasses + repository ports + Postgres adapter (datasource-exchangeable); ORM tables stay in models.py; items.art is a FK to arts exposed as a string. Also covers vendors/components/systems lifecycle tracking (EoS/EoL/EoS-support dates, risk rollup from component → system → service) surfaced in the product Components/Systems tabs, the service drawer's Provided-by block, and the cross-product Lifecycle view. Also covers support contracts ([routers/contracts.py](backend/app/routers/contracts.py)) — product-scoped (optional vendor), linkable to 0..n components, with notice-period-derived expiry status — plus component budget fields (yearly run cost, replacement budget), surfaced in the product Contracts tab and the cross-product Contracts view. Also covers product roadmaps ([routers/roadmap.py](backend/app/routers/roadmap.py)) — product-scoped streams (ordered swimlanes) holding roadmap items (status + date range), linkable to 0..n features, surfaced in the Roadmap view; roadmap feature links (and PI-objective feature links) CASCADE-delete with their feature and are not captured by snapshots, so they do not survive a CSV re-import or a snapshot restore.

**Frontend** (React + Vite + TypeScript + Tailwind v4) — [frontend/src/](frontend/src/):
- [App.tsx](frontend/src/App.tsx) wires up react-router v7 routes for the nine top-level views — `/board`, `/planning`, `/timeline`, `/ranking`, `/products` (plus `/products/:productId`), `/lifecycle`, `/contracts`, `/roadmap`, and `/admin/:section` (admin-only) — behind the collapsible grouped [shell/Sidebar](frontend/src/shell/Sidebar.tsx) (Work/Catalog/Admin groups, config in [shell/nav.ts](frontend/src/shell/nav.ts)). Every view opens with [shell/PageHeader](frontend/src/shell/PageHeader.tsx) as its first row. The Work views and Admin nest under [shell/WorkLayout](frontend/src/shell/WorkLayout.tsx), a route layout holding shared board state and the ItemDrawer stack via outlet context; catalog views (Products, Lifecycle, Contracts, Roadmap) mount directly. All backend calls go through the typed client in [api/client.ts](frontend/src/api/client.ts); shared types in [types.ts](frontend/src/types.ts) mirror the backend schemas.
...
- Custom dropdown components are deliberate: `PlainSelect` (no search), `SearchableSelect` (filterable), `FilterSelect` (filter bars). These are hand-built Tailwind popovers — do not swap in native `<select>`. Drag-and-drop uses `@dnd-kit`. The shared primitive set in [components/](frontend/src/components/) — `Badge`, `Banner`, `EmptyState`, the `Skeleton` set, `SegmentedToggle`, `TogglePill`, `TabBar`, `Menu`, `DrawerShell` — is likewise deliberate; reach for these before hand-rolling equivalent markup. Status chips are `<Badge>` — never hand-rolled spans. Stacking order follows the z-ladder tokens in [ui.ts](frontend/src/components/ui.ts): `zPopover` (20) < `zDrawer` (40) < `zModal` (50) < `zConfirm` (60) — new overlay-ish surfaces pick the token for their layer rather than a bare `z-*` utility. Every caption/field label uses the shared `captionClass` token rather than a hand-copied class string. Content scroll containers are `px-6 py-4` (Timeline's sticky-header lane is the one documented exception). Create-button labels ("New Feature", "Add stream", "Add person", …) carry no `+` prefix.
...
Alembic migrations in [backend/alembic/versions/](backend/alembic/versions/) (head is `0029`). SQLite unit-test fixtures never exercise the Alembic DDL, so before accepting a migration, dry-run **both** `alembic upgrade head` and `alembic downgrade -1` against the compose Postgres. Postgres is v18 and the backend image ships `postgresql-client-18` so `pg_dump` matches the server.
