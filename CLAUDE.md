...
- [App.tsx](frontend/src/App.tsx) is the shell with nine top-level views: Board, Planning, Timeline, Ranking, Products, Lifecycle, Contracts, Roadmap, and Admin (admin-only). All backend calls go through the typed client in [api/client.ts](frontend/src/api/client.ts); shared types in [types.ts](frontend/src/types.ts) mirror the backend schemas.
