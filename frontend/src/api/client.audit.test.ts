...
it("getItemEvents fetches the item's events", async () => {
  ...
  expect(spy.mock.calls[0][0]).toBe("/api/v1/items/7/events");
});
...
it("getAuditEvents builds the query string and skips empty params", async () => {
  ...
  expect(spy.mock.calls[0][0]).toBe("/api/v1/audit?limit=50&offset=100&q=marco&entity_type=auth");

  const spy2 = mockFetch(200, { items: [], total: 0 });
  await getAuditEvents({});
  expect(spy2.mock.calls[0][0]).toBe("/api/v1/audit");
});
