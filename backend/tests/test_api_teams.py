...
def test_create_list_delete_team(client):
    resp = client.post("/api/v1/teams", json={"name": "Network"})
    assert resp.status_code == 201
    team_id = resp.json()["id"]
    assert [t["name"] for t in client.get("/api/v1/teams").json()] == ["Network"]
...
def test_duplicate_team_returns_409(client):
    ...
    assert client.post("/api/v1/teams", json={"name": "Network"}).status_code == 409
