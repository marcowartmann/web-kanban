...
def test_delete_logs_item_and_cascaded_children(client, db_session):
    feature_id = client.post("/api/v1/items", json={"kind": "feature", "title": "F"}).json()["id"]
    client.post("/api/v1/items", json={"kind": "story", "title": "S", "parent_id": feature_id})
    assert client.delete(f"/api/v1/items/{feature_id}").status_code == 204
...
def test_item_events_endpoint_newest_first_and_scoped(client, db_session):
    ...
    assert client.get(f"/api/v1/items/{b}/events").json()[0]["event_type"] == "item.created"
    assert client.get("/api/v1/items/999/events").status_code == 404


def test_member_can_read_item_events(member_client, db_session):
    item_id = member_client.post("/api/v1/items", json={"kind": "feature", "title": "M"}).json()["id"]
    events = member_client.get(f"/api/v1/items/{item_id}/events").json()
...
def test_max_length_title_still_creates_and_audits(client, db_session):
    ...
    resp = client.post("/api/v1/items", json={"kind": "feature", "title": title})
    assert resp.status_code == 201
