...
def test_team_and_pi_events(client, db_session):
...
def test_capacity_set_logs_old_and_new(client, db_session):
    user_id = client.post("/api/v1/users", json={"display_name": "Marco"}).json()["id"]
    client.put(
        "/api/v1/capacities",
        json={"user_id": user_id, "planning_interval": "PI1", "iteration": 2, "points": 3},
    )
    ...
    client.put(
        "/api/v1/capacities",
        json={"user_id": user_id, "planning_interval": "PI1", "iteration": 2, "points": 5},
    )
