...
def test_admin_pagination_and_total(client, db_session):
    ...
    page = client.get("/api/v1/audit?limit=3&offset=0").json()
    assert page["total"] == 7
    assert len(page["items"]) == 3
    # newest first: highest entity_id seeded last comes first
    assert page["items"][0]["entity_id"] == 7
    page2 = client.get("/api/v1/audit?limit=3&offset=6").json()
...
def test_q_and_entity_type_filters(client, db_session):
    ...
    by_type = client.get("/api/v1/audit?entity_type=auth").json()
    assert by_type["total"] == 1
    assert by_type["items"][0]["event_type"] == "auth.login_failed"

    by_q = client.get("/api/v1/audit?q=alpha").json()
    assert by_q["total"] == 2

    by_q_event = client.get("/api/v1/audit?q=login_failed").json()
    assert by_q_event["total"] == 1


def test_limit_clamped(client, db_session):
    _seed_events(db_session, n=1)
    assert client.get("/api/v1/audit?limit=9999").status_code == 200
    assert client.get("/api/v1/audit?limit=0").status_code == 200
