...
def test_team_rename_propagates_to_items(client, db_session):
    team = client.post("/api/v1/teams", json={"name": "Network"}).json()
    ...
    resp = client.patch(f"/api/v1/teams/{team['id']}", json={"name": "Net & Cloud"})
    assert resp.status_code == 200
    assert resp.json()["name"] == "Net & Cloud"
    assert client.get(f"/api/v1/items/{lead}").json()["leading_team"] == "Net & Cloud"
    assert client.get(f"/api/v1/items/{support}").json()["supporting_team"] == "Net & Cloud"
    assert client.get(f"/api/v1/items/{other}").json()["leading_team"] == "Platform"
...
def test_team_rename_conflicts_and_noop(client, db_session):
    a = client.post("/api/v1/teams", json={"name": "A"}).json()
    client.post("/api/v1/teams", json={"name": "B"})

    dup = client.patch(f"/api/v1/teams/{a['id']}", json={"name": "B"})
    assert dup.status_code == 409
    assert dup.json()["detail"] == "Team already exists"

    noop = client.patch(f"/api/v1/teams/{a['id']}", json={"name": "A"})
    assert noop.status_code == 200
    assert _events(db_session, "team.renamed") == []

    missing = client.patch("/api/v1/teams/9999", json={"name": "X"})
    assert missing.status_code == 404
    assert missing.json()["detail"] == "Team not found"


def test_team_delete_guard_and_force(client, db_session):
    team = client.post("/api/v1/teams", json={"name": "Guarded"}).json()
...
def test_pi_rename_propagates_items_and_capacities(client, db_session):
    ...
    resp = client.patch(f"/api/v1/planning-intervals/{pi['id']}", json={"name": "PI1-Q3"})
    assert resp.status_code == 200
    assert client.get(f"/api/v1/items/{item}").json()["planning_interval"] == "PI1-Q3"
...
def test_pi_delete_guard_counts_items_and_capacities(client, db_session):
    ...
    blocked = client.delete(f"/api/v1/planning-intervals/{pi['id']}")
    assert blocked.status_code == 409
    assert blocked.json()["detail"] == (
        "Planning interval 'PI9' is used by 1 items and 1 capacity entries"
    )
    assert client.delete(f"/api/v1/planning-intervals/{pi['id']}?force=true").status_code == 204


def test_pi_rename_duplicate_409(client):
    a = client.post("/api/v1/planning-intervals", json={"name": "P-A"}).json()
    client.post("/api/v1/planning-intervals", json={"name": "P-B"})
    dup = client.patch(f"/api/v1/planning-intervals/{a['id']}", json={"name": "P-B"})
    assert dup.status_code == 409
...
def test_lane_rename_propagates_scoped_by_board_kinds(client, db_session):
    ...
    risk = client.post(
        "/api/v1/items", json={"kind": "risk", "title": "R", "status": "New"}
    ).json()["id"]

    resp = client.patch(f"/api/v1/lanes/{new_lane['id']}", json={"name": "Fresh"})
    assert resp.status_code == 200
    assert client.get(f"/api/v1/items/{risk}").json()["status"] == "Fresh"
    assert client.get(f"/api/v1/items/{story_parent}").json()["status"] == "New"
    assert _events(db_session, "item.updated") == []
